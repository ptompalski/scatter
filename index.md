...
systematic patterns, biases, or outliers. To address this,
[`scatter()`](https://ptompalski.github.io/scatter/reference/scatter.md)
supports density-colored points
...
For convenience,
[`scatter()`](https://ptompalski.github.io/scatter/reference/scatter.md)
can also switch to this behavior automatically using
`point_style = "auto"` (default), which enables density coloring once
the number of observations exceeds a configurable threshold.

## Axis orientation: observed vs. predicted

By default,
[`scatter()`](https://ptompalski.github.io/scatter/reference/scatter.md)
places observed (truth) values on the x-axis and predicted (estimate)
values on the y-axis. This follows recommendations in the modelling
literature, but users can easily switch back to the alternative
convention (predicted on x) using `swap_axes = TRUE`.
...
Importantly, axis orientation in
[`scatter()`](https://ptompalski.github.io/scatter/reference/scatter.md)
affects only the visualization; agreement metrics are always computed
using the original (truth, estimate) pairing (compare the figure below
with the top-most scatterplot):
